...
def get_genres(
    ...
    try:
        ...
        raise KeyError(
            "Using the get_genres module requires the config option USB_PATH"
        ) from KeyError
    ...
    with ThreadPoolExecutor(max_workers=cpu_count() * 4) as executor:
        tracks = [
            y for x in tqdm(executor.map(get_tag, *payload), total=len(files))
            for y in x
        ]

    for group_id, group in groupby(
        sorted(tracks, key=itemgetter(0)), key=itemgetter(0)
    ):
        group = list(group)
...
def get_tag(
    _file: str, config: Dict[str, Union[List, Dict, str, bool, int, float]]
) -> List[Tuple[str]]:
    """Loads mp3 file and reads the "genre" ID3 tag field. The genre tag is
        split on "GENRE_TAG_DELIMITER" and each individual genre tag is zipped
        with the file name.

    Args:
        _file: Path to mp3 file.
        config: Configuration object.

    Returns:
        List of (genre tag, track title) tuples.
    """
    genres = set(
        map(
            clean_tag,
...
def clean_tag(tag: str) -> str:
    ...
    Args:
        tag: Uncleaned genre tag.

    Returns:
        Genre tag cleaned of garbage characters.
    """
