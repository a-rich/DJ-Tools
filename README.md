...
`NOTE`: to ensure Collection consistency and successful operation of `DJ Tools`, the following properties should be maintained for all music files. Users of my beatcloud _must_ complete a minimum of (1) and (2) prior to uploading, but it is greatly appreciated if they also complete (3) and (4). Futhermore, it is advised they complete (5), (6), and (7) as well, although these are steps I expect to redo myself when integrating others' tracks:
...
6. `comment` tags 1st cleared and then populated with important information
    - BPM changes
